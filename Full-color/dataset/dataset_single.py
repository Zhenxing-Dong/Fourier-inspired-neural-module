...
def get_training_data(opt):
    ...
    image_res = (1072, 1920)
    roi_res = (880, 1600)
    data_path = os.path.join(opt.data_dir, 'train')
    print('data_path is ', data_path)
    dataloader = ImageLoader(data_path, channel=opt.channel,
                            image_res=image_res, homography_res=roi_res, batch_size=opt.batch_size,
                            crop_to_homography=False, shuffle=True, 
                            vertical_flips=True, horizontal_flips=True)
...
def get_validation_data(opt):
    ...
    image_res = (1072, 1920)
    roi_res = (880, 1600)
    data_path = os.path.join(opt.data_dir, 'val')
    print('data_path is ', data_path)
    dataloader = ImageLoader(data_path, opt.channel,
                            image_res=image_res, homography_res=roi_res, batch_size=opt.batch_size,
                            crop_to_homography=False, shuffle=False, 
...
def get_test_data(opt):
    ...
    image_res = (1072, 1920)
    roi_res = (880, 1600)
    data_path = os.path.join(opt.data_dir, 'test')
    print('data_path is ', data_path)
    dataloader = ImageLoader(data_path, channel=opt.channel,
                             image_res=image_res, homography_res=roi_res, batch_size=opt.batch_size,
                             crop_to_homography=False, shuffle=False, 
