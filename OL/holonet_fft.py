...
from algorithms import double_phase_amp_phase
...
class HoloNet(nn.Module):
    ...
    """
    def __init__(self, distance=0.1, wavelength=543e-9, feature_size=3.74e-6,
    ...
    def forward(self, target_amp):
        ...
        if self.final_phase_only is None:
            return amp, double_phase_amp_phase(amp, ang, three_pi=False)
        else:
...
class InitialPhaseUnet_FFT(nn.Module):
    def __init__(self, bilinear=False, dim=16):
        super(InitialPhaseUnet_FFT, self).__init__()
...
class FinalPhaseOnlyUnet_FFT(nn.Module):
    def __init__(self, bilinear=False, dim=16):
        super(FinalPhaseOnlyUnet_FFT, self).__init__()
        self.bilinear = bilinear
        self.dim = dim
        self.inc = FirstConv(2, dim)
        self.down1 = Down(dim, dim*2)
        self.down2 = Down(dim*2, dim*4)
        self.down3 = Down(dim*4, dim*8)
        factor = 2 if bilinear else 1
