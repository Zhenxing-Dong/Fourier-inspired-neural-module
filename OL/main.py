...
from holonet import HoloNet, InitialPhaseUnet, FinalPhaseOnlyUnet, PhaseOnlyUnet, InitialPhaseUnet_FFT, FinalPhaseOnlyUnet_FFT
...
p.add_argument('--data_path', type=str, default='./data/eval', help='Directory for the dataset')
...
wavelength = (638 * nm, 543 * nm, 450 * nm)[channel]  # wavelength of each color
feature_size = (3.74 * um, 3.74 * um)  # SLM pitch
slm_res = (1072, 1920)  # resolution of SLM
image_res = (1072, 1920)
roi_res = (1072, 1920)  # regions of interest (to penalize for SGD)
...
if opt.method == 'SGD':
    ...
    model_path = os.path.join(opt.generator_dir, 'holonet20_green.pth')
    image_res = (1072, 1920)

elif opt.method == 'HOLONET_all_fft':
    print('HOLONET_all_fft')
    phase_only_algorithm = HoloNet(prop_dist, wavelength, feature_size, initial_phase=InitialPhaseUnet_FFT(),
                                   final_phase_only=FinalPhaseOnlyUnet_FFT()).to(device)
    model_path = os.path.join(opt.generator_dir, 'holonet_all_fft_green_2022-11-09-20-53-45.699358_5.pth')
    image_res = (1072, 1920)

elif opt.method == 'HOLONET_final_fft':
    print('HOLONET_final_fft')
    phase_only_algorithm = HoloNet(prop_dist, wavelength, feature_size, initial_phase=InitialPhaseUnet(4, 16),
                                   final_phase_only=FinalPhaseOnlyUnet_FFT()).to(device)
    model_path = os.path.join(opt.generator_dir, 'holonet_final_fft_green_green_37.pth')
    image_res = (1072, 1920)

elif opt.method == 'HOLONET_inital_fft':
    print('HOLONET_inital_fft')
    phase_only_algorithm = HoloNet(prop_dist, wavelength, feature_size, initial_phase=InitialPhaseUnet_FFT(),
                                   final_phase_only=FinalPhaseOnlyUnet(4, 16, num_in=2)).to(device)
    model_path = os.path.join(opt.generator_dir, 'holonet_inital_fft_green_green_39.pth')
    image_res = (1072, 1920)


elif opt.method == 'UNET_fft':
    phase_only_algorithm = UNet_fft().to(device)
    model_path = os.path.join(opt.generator_dir, 'model_best_green.pth')
    image_res = (1072, 1920) 
...
for k, target in enumerate(image_loader):
    ...
    if opt.method in ['DPAC', 'HOLONET', 'UNET', 'UNET_fft', 'Holoencoder', 'HOLONET_all_fft', 'HOLONET_final_fft', 'HOLONET_inital_fft', 'naive_UNET']:
        # direct methods
        start = time.time()
    ...
    # init_phase_8bit = utils.phasemap_8bit(init_phase.cpu().detach(), inverted=True)

    utils.cond_mkdir(root_path)
    cv2.imwrite(os.path.join(root_path, f'{target_idx}.png'), phase_out_8bit)
    # cv2.imwrite(os.path.join('./init_phase', f'{target_idx}.png'), init_phase_8bit)
